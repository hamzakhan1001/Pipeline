...
class Brand
{
    ...
    {
        if (empty($this->brandName) || empty($output)) {
            return $output;
        }

        $tagManangerMatomoId = '"id":"Matomo"';
        $noBrandReplace = array('Piwik_', 'rssPiwik', 'loadingPiwik', 'Matomo_', 'rssMatomo', 'loadingMatomo', $tagManangerMatomoId, 'MatomoTagManager');
        $temporaryReplace = array('__##PIWIK##__', '__##RSSPIWIK##__', '__##LOADINGPIWIK##__', '__##MATOMO##__', '__##RSSMATOMO##__', '__##LOADINGMATOMO##__', '__##MATOMOID##__', '__##MatomoTagManager##__');
    ...
    public function removeMobileAppAd($output)
    {
        if (!empty($output)) {
            ...
            $output = str_replace('<meta name="description" content="free/libre analytics platform"/>', '<meta name="description" content="The HIPAA compliant website analytics purpose built for healthcare."/>', $output);
