...
class Dimensions
{
    ...
    public function check()
    {
        ...
        $availableDimensions = $this->getAvailableDimensionsList();
        foreach ($this->dimensions as $index => $dimension) {
            ...
            if ($validateDimensionsExist && !isset($availableDimensions[$dimension])) {
                throw new Exception(Piwik::translate('CustomReports_ErrorInvalidDimension', array($dimension)));
            }
        }
    ...
    private function getAvailableDimensionsList()
    {
        $dimensions = API::getInstance()->getAvailableDimensions($this->idSite);
        $list = [];
            ...
            foreach ($dimensions as $values) {
                if (!empty($values['dimensions'])) {
                    foreach ($values['dimensions'] as $dimension) {
                        $list[$dimension['uniqueId']] = $dimension['name'];
                    }
                }
            }
        }
