...
class CustomReport extends RecordBuilder
{
     ...
     * @var bool
     */
    private $doneFirstLevel = false;

    /**
     * @var bool
     */
    private $doneSecondLevel = false;
    ...
    public function __construct(array $report, DimensionsProvider $dimension, Configuration $configuration)
    {
        ...
        $this->maxRowsInTable = $this->configuration->getArchiveMaxRows();
        $this->maxRowsInSubtable = $this->configuration->getArchiveMaxRowsSubtable();
    ...
    public function aggregate(ArchiveProcessor $archiveProcessor): array
    {
        ...
        $this->doneFirstLevel = false;
        $this->doneSecondLevel = false;
    ...
    public function aggregateReport(ArchiveProcessor $archiveProcessor, int $idSite, $onlyMetrics = array()): array
    {
        $report = $this->report;
        ...
        foreach ($dimensionsPlan as $dimensionsGroup) {
            ...
            if (count($dimensionsGroup['left']) === 1) {
                $this->doneFirstLevel = true;
            } elseif (count($dimensionsGroup['left']) === 2) {
                $this->doneFirstLevel = true;
                $this->doneSecondLevel = true;
            }
        }
    ...
    private function makeRegularReport(DataTable $mainRecord, array &$records, array $dimensionsInThisRun, $cursor, int $idSite): void
    {
        ...
        while ($row = $cursor->fetch()) {
            ...
            $secondLevelLabel = Archiver::LABEL_NOT_DEFINED;
            $thirdLevelLabel = Archiver::LABEL_NOT_DEFINED;

            if (isset($row[$firstLevelColumn])) {
                $firstLevelLabel = $row[$firstLevelColumn];
                unset($row[$firstLevelColumn]);

                if ($this->isEmptyLabel($firstLevelLabel)) {
                    $firstLevelLabel = Archiver::LABEL_NOT_DEFINED;
                }
            }
            ...
            if (isset($secondLevelColumn) && isset($secondLevelColumn) && isset($row[$secondLevelColumn])) {
                ...
                if ($this->isEmptyLabel($secondLevelLabel)) {
                    $secondLevelLabel = Archiver::LABEL_NOT_DEFINED;
                }
            }
            ...
            if (isset($thirdLevelColumn) && isset($thirdLevelColumn) && isset($row[$thirdLevelColumn])) {
                ...
                if ($this->isEmptyLabel($thirdLevelLabel)) {
                    $thirdLevelLabel = Archiver::LABEL_NOT_DEFINED;
                }
            }
            ...
            if ($numDimensions === 1 || !$this->doneFirstLevel) {
                $columns['level'] = 1;
                $mainRecord->sumRowWithLabel($firstLevelLabel, $columns, $this->columnAggregationOps);
            }

            if ($numDimensions === 2 || (!$this->doneSecondLevel && $numDimensions > 2)) {
                $columns['level'] = 2;
                $topLevelRow = $mainRecord->sumRowWithLabel($firstLevelLabel, []);
                $topLevelRow->sumRowWithLabelToSubtable($secondLevelLabel, $columns, $this->columnAggregationOps);
            }

            if ($numDimensions === 3) {
                $columns['level'] = 3;
                $topLevelRow = $mainRecord->sumRowWithLabel($firstLevelLabel, []);
                $secondLevelRow = $topLevelRow->sumRowWithLabelToSubtable($secondLevelLabel, []);
                $secondLevelRow->sumRowWithLabelToSubtable($thirdLevelLabel, $columns, $this->columnAggregationOps);
            }
        }
