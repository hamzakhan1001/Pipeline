...
class Controller extends PluginController
{
    ...
    public function formSummary()
    {
        ...
        $form = $this->getForm($this->idSite, $idSiteForm);

        $canEditForm = $this->validator->canWrite($this->idSite);
        $isVisitorLogEnabled = Manager::getInstance()->isPluginActivated('Live') && Live::isVisitorLogEnabled();
        ...
        return $this->renderTemplate('formSummary', array(
            ...
            'isVisitorLogEnabled' => $isVisitorLogEnabled,
        ));
    }
    ...
    public function getEvolutionGraph()
    {
        ...
        if (!empty($idSiteForm)) {
            $this->getForm($this->idSite, $idSiteForm); // check form exists
        }
    ...
    private function getForm($idSite, $idForm)
    {
        $form = Request::processRequest('FormAnalytics.getForm', [
            'idSite' => $idSite,
            'idForm' => $idForm,
        ], $default = []);
        if (empty($form)) {
