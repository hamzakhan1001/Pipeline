...
abstract class Base extends Report
{
    ...
    protected function setDefaultMetrics()
    {
        ...
        if (!$hasRequestedSecondaryDimension) {
            $hasRequestedSecondaryDimension = $secondaryDimension;
        }

        if ($hasRequestedSecondaryDimension) {
            if ($secondaryDimension === Archiver::SECONDARY_DIMENSION_MEDIA_SEGMENTS) {
                $this->metrics = array(Metrics::METRIC_NB_PLAYS);
    ...
    public function setExpandableTable(ViewDataTable $view)
    {
        ...
        } elseif ($isDetailPage && $view->isViewDataTableId(MediaEvolution::ID)) {
            $view->config->columns_to_display = array(Metrics::METRIC_PLAY_RATE);
            $view->config->filters[] = array(function () use ($view) {
                $view->config->columns_to_display = array(Metrics::METRIC_PLAY_RATE);
            }, $params = array(), $priority = true);
        }
    ...
    public function setMetricTranslations(ViewDataTable $view)
    {
        $view->config->metrics_documentation[Metrics::METRIC_NB_IMPRESSIONS] = Piwik::translate('MediaAnalytics_ColumnDescriptionImpressions');
    }
    ...
    public function configureReportMetadata(&$availableReports, $infos)
    {
        ...
        // We have to manually set this one so that it can't get overridden by SEKP
        $report['metricsDocumentation'][Metrics::METRIC_NB_IMPRESSIONS] = Piwik::translate('MediaAnalytics_ColumnDescriptionImpressions');
