...
class VideoPlays extends Base
{
    public function getRecordMetadata(ArchiveProcessor $archiveProcessor): array
    {
        return [
            Record::make(Record::TYPE_NUMERIC, Archiver::NUMERIC_RECORD_PREFIX . Metrics::METRIC_TOTAL_VIDEO_PLAYS),
        ];
    }
    ...
    protected function aggregate(ArchiveProcessor $archiveProcessor): array
    {
        ...
        $cursor = $aggregator->query($select, $where = 'AND watched_time > 0 AND media_type = ' . MediaAnalytics::MEDIA_TYPE_VIDEO, $groupBy = '', $orderBy = '');
        $row = $cursor->fetch();

        $recordName = Archiver::NUMERIC_RECORD_PREFIX . Metrics::METRIC_TOTAL_VIDEO_PLAYS;

        return [
            $recordName => $row[Metrics::METRIC_TOTAL_VIDEO_PLAYS],
        ];
    }
}
