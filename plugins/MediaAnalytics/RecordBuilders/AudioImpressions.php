...
class AudioImpressions extends Base
{
    public function getRecordMetadata(ArchiveProcessor $archiveProcessor): array
    {
        return [
            Record::make(Record::TYPE_NUMERIC, Archiver::NUMERIC_RECORD_PREFIX . Metrics::METRIC_TOTAL_AUDIO_IMPRESSIONS),
        ];
    }
    ...
    protected function aggregate(ArchiveProcessor $archiveProcessor): array
    {
        ...
        $cursor = $aggregator->query($select, $where = ' AND media_type = ' . MediaAnalytics::MEDIA_TYPE_AUDIO, $groupBy = '', $orderBy = '');
        $row = $cursor->fetch();

        $recordName = Archiver::NUMERIC_RECORD_PREFIX . Metrics::METRIC_TOTAL_AUDIO_IMPRESSIONS;

        return [
            $recordName => $row[Metrics::METRIC_TOTAL_AUDIO_IMPRESSIONS],
        ];
    }
}
