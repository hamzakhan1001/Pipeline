...
class Targets
{
    ...
    public function check()
    {
        if ($this->needsAtLeastOneEntry && empty($this->targets)) {
            throw new Exception(Piwik::translate('AbTesting_ErrorXNotProvided', $this->parameterName));
        }

        if (!is_array($this->targets)) {
            throw new Exception(Piwik::translate('AbTesting_ErrorNotAnArray', $this->parameterName));
        }

        foreach ($this->targets as $index => $target) {
            $target = new Target($target, $this->parameterName, $index);
            $target->check();
        }
    }
}
