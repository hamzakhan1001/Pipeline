    ...
    public function aggregateTopStepActions($step, $dataSource, $ignoreSearchQuery, $ignoreDomain, $siteKeepsUrlFragments, $exploreStep, $exploreValueToMatch)
    {
        ...
        if ($doExploreTraffic) {
            ...
            if ($isExploringSearch) {
                $extraWhere .= ' 
                    AND log_link_visit_action_match.pageview_position = ' . (int) $exploreStep . ' AND log_link_visit_action_match.' . $columnToFetch . ' is null';
            } else {
                $columnToExplore = $this->makeUrlColumn('`log_action_match`.`name`', $dataSource, $ignoreSearchQuery, $ignoreDomain, $siteKeepsUrlFragments);
                $extraWhere .= ' 
                    AND log_link_visit_action_match.pageview_position = ' . (int) $exploreStep . " AND $columnToExplore = $keyToBeReplacedWithBind";
                $from[] = array(
                    'table'  => 'log_action',
        ...
        $from[] = array(
            ...
            'joinOn' => "log_link_visit_action.idvisit = log_link_visit_action_next.idvisit 
                            AND log_link_visit_action_next.pageview_position = " . $nextStep
        );
        ...
        $where .= " AND log_link_visit_action.pageview_position = $step " . $extraWhere;
