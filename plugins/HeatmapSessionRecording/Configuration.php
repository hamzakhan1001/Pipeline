...
class Configuration
{
    ...
    public function canViewSystemReport()
    {
        $config = $this->getConfig();
        if (!isset($config->General[self::KEY_CAN_VIEW_SYSTEM_REPORT_GENERAL])) {
            return true;
        }

        return (bool) $config->General[self::KEY_CAN_VIEW_SYSTEM_REPORT_GENERAL];
    }
