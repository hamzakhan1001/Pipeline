...
class GettingStartedHeatmap extends Widget
{
    public static function configure(WidgetConfig $config)
    {
        ...
        if (self::shouldEnable($idSite) && !self::getAccessValidator()->isHeatmapRecordingDisabled()) {
            $config->enable();
    ...
    private static function shouldEnable($idSite)
    {
        ...
        return !empty($idSite) && $validator->canViewHeatmapReport($idSite) && !$validator->canWrite($idSite);
    }
    ...
    public function render()
    {
        ...
        self::getAccessValidator()->checkHeatmapReportViewPermission($idSite);

        if (self::shouldEnable($idSite)) {
            return $this->renderTemplate('gettingStartedHeatmaps');
        }
